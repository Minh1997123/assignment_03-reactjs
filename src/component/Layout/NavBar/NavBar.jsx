import style from "./NavBar.module.css";
import { NavLink, useNavigate, useLocation } from "react-router";
import { useSelector, useDispatch } from "react-redux";
import { useState, useEffect } from "react";
import { onLogout } from "../../../store/Slice";
const NavBar = function () {
  const [nameUser, setNameUser] = useState(null);
  const navigate = useNavigate();
  const location = useLocation();
  const loginData = useSelector((state) => state.login);
  const dispatch = useDispatch();
  // ham xac dinh hien thi patch dang hoat dong
  const activeStyle = ({ isActive }) => {
    return isActive && style.active;
  };
  //   ham chuyen huong bang navigate
  const navigateHandler = function () {
    navigate("/");
  };
  // ham logout
  const logoutHandler = function () {
    dispatch(onLogout());
    setNameUser(null);
  };
  useEffect(
    function () {
      // kiem tra xem co nguoi dang nhap ko
      if (!loginData.isLogin) {
        return;
      }
      // lay ten viet tat cua nguoi dang nhap de hien thi
      const fullName = loginData.userCurrent.fullName;
      const nameArr = fullName.split(" ");
      const lastNameAcronym = nameArr[0].split("", 1)[0];
      const firstName = nameArr[nameArr.length - 1];
      const nameUserCurrent = firstName + lastNameAcronym;
      setNameUser(nameUserCurrent);
    },
    [loginData]
  );
  return (
    <nav className={style.navbar}>
      <div className={style.navbar__button}>
        <button
          onClick={navigateHandler}
          className={location.pathname === "/" ? style.active : ""}
        >
          Home
        </button>
        <NavLink to="/shop" className={activeStyle}>
          Shop
        </NavLink>
      </div>
      <h1>BOUTIQUE</h1>
      <div className={style.navbar__button}>
        <NavLink to="/cart" className={activeStyle}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
            <path
              fill="#aaaaaa"
              d="M32 0C14.3 0 0 14.3 0 32S14.3 64 32 64l16 0c8.8 0 16 7.2 16 16l0 288c0 44.2 35.8 80 80 80l18.7 0c-1.8 5-2.7 10.4-2.7 16c0 26.5 21.5 48 48 48s48-21.5 48-48c0-5.6-1-11-2.7-16l197.5 0c-1.8 5-2.7 10.4-2.7 16c0 26.5 21.5 48 48 48s48-21.5 48-48c0-5.6-1-11-2.7-16l66.7 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-464 0c-8.8 0-16-7.2-16-16l0-288C128 35.8 92.2 0 48 0L32 0zM192 80l0 192c0 26.5 21.5 48 48 48l320 0c26.5 0 48-21.5 48-48l0-192c0-26.5-21.5-48-48-48l-96 0 0 144c0 5.9-3.2 11.3-8.5 14.1s-11.5 2.5-16.4-.8L400 163.2l-39.1 26.1c-4.9 3.3-11.2 3.6-16.4 .8s-8.5-8.2-8.5-14.1l0-144-96 0c-26.5 0-48 21.5-48 48z"
            />
          </svg>
          Cart
        </NavLink>
        {!loginData.isLogin ? (
          <NavLink to="/login" className={activeStyle}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
              <path
                fill="#aaaaaa"
                d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304l-91.4 0z"
              />
            </svg>
            Login
          </NavLink>
        ) : (
          <span className={style.logout}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
              <path
                fill="#aaaaaa"
                d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304l-91.4 0z"
              />
            </svg>
            {nameUser}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
              <path d="M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z" />
            </svg>
            <button onClick={logoutHandler}>(Logout)</button>
          </span>
        )}
      </div>
    </nav>
  );
};
export default NavBar;

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
  <path d="M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z" />
</svg>;
